...
%%
%% result = Test_ALMSNCG_real_miter(prob_vec,tau,C,tol,stop_flag,maxiter_vec,datadir)
%%
%% INPUT:
...
%====================================================================================
function result = Test_ALMSNCG_real_miter(prob_vec,tau,C,tol,stop_flag,maxiter_vec,datadir)
...
for pp = 1:lenp
    ...
    OPTIONS.ifrandom = 0;
    OPTIONS.test_svd = 1; % Test time of SVD
    ...
    for mm = 1:lenmiter
        OPTIONS.maxiter = maxiter_vec(mm);
        ...
        result(mm+(pp-1)*lenmiter,1) = OPTIONS.n;
        result(mm+(pp-1)*lenmiter,2) = OPTIONS.p;
        result(mm+(pp-1)*lenmiter,3) = OPTIONS.q;
        result(mm+(pp-1)*lenmiter,4) = OPTIONS.tol;
        result(mm+(pp-1)*lenmiter,5) = OPTIONS.tau;
        result(mm+(pp-1)*lenmiter,6) = OPTIONS.C;
        result(mm+(pp-1)*lenmiter,7) = info.r_indexJ;
        result(mm+(pp-1)*lenmiter,8) = info.k1;
        result(mm+(pp-1)*lenmiter,9) = accuracy_test;
        if OPTIONS.stop == 1
            result(mm+(pp-1)*lenmiter,10) = info.relobj;
        end
        result(mm+(pp-1)*lenmiter,11) = info.totaltime;
        result(mm+(pp-1)*lenmiter,12) = info.iter;
        result(mm+(pp-1)*lenmiter,13) = info.numSSNCG;
        if OPTIONS.test_svd
            result(mm+(pp-1)*lenmiter,14) = info.num_svd;
            result(mm+(pp-1)*lenmiter,15) = info.num_svds;
            result(mm+(pp-1)*lenmiter,16) = info.num_svd+info.num_svds;
            result(mm+(pp-1)*lenmiter,17) = info.time_svd; % time spent on SVD
        end
        result(mm+(pp-1)*lenmiter,18) = obj(1);

    end
end

%save result_alm_miter.mat result
