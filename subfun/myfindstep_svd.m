...
function [W_snew,b_snew,v_snew,U_snew,lam_snew,Lam_snew,omega_new,X_new,Pomega_new,PX_new,alpha,iter,g0,Ly,maxiterfs,AdW,normvsqr,normUsqr,FnormW_Bsqr_new,parPX] = ...
    myfindstep_svd(A,y0,y,W_sub,b_sub,v_sub,U_sub,lam_sub,Lam_sub,omega_sub,X_sub,dW,db,gradphiW,gradphib,Ly0,FnormW_Bsqr,tol,AJdW,parsub)
%%
printlevel = 0;
...
num_svd = 0; num_svds = 0;
if parsub.test_svd, time_svd = 0; end
%%
...
if (g0 <= 0)
    ...
    if flag_tau
        ...
        if parsub.test_svd
            [PX_new,parPX] = projspball_svd(X_sub,tau);
            time_svd = time_svd + parPX.time_svd; parPX.time_svd = time_svd;
        else
...
for iter = 1:maxit
    ...
    if flag_tau
        ...
        if ((parsub.flag_svds == 1) && (iter > 1)) || ((iter >= 5) && all(k1_vec(iter-4:iter-2)) == k1_vec(iter-1))
            if parsub.test_svd
                [PX_new, parPX] = projspball_svd(X_new,tau,k1_vec(iter-1));
            ...
            if parsub.test_svd
                [PX_new, parPX] = projspball_svd(X_new,tau);
                time_svd = time_svd + parPX.time_svd;
            else
    ...
    if (iter == 1)
        ...
        if (sign(gLB)*sign(gUB) > 0)
            ...
            if flag_tau
                ...
                Lam_snew = PX_new;
                parPX.num_svd = num_svd; parPX.num_svds = num_svds; 
                if parsub.test_svd, parPX.time_svd = time_svd; end
            else
